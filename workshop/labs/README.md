...
**인바운드(하나카드=매입사) 재반박(Acquirer Dispute Response / 매입사 방어)** 을 도와주는 서비스입니다. 가맹점 자료를 넣으면 → 사유코드·필수증빙을 안내하고 → **재반박서(영문) 초안**을 만들어 줍니다. 그 "두뇌"는 우리가 직접 만드는 **차지백 스킬**(Lab 3)이고, 이를 **AgentCore 에이전트**로 배포해 **Next.js 웹**에서 씁니다.
...
| **Lab 3** | 차지백 이해 스킬 만들기 | 11:05–12:00 | `skill-creator`로 재반박 도메인 스킬 제작 | [`03-build-chargeback-skill.md`](./03-build-chargeback-skill.md) |
