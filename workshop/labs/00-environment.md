...
**예상 소요시간:** 약 15분 (워크숍 시작 직후, 10:00–10:15 · SA와 화면을 함께 보며 진행)

> ℹ️ **참고:** 오늘은 설치하는 날이 아닙니다. 설치(Claude Code + Amazon Bedrock 연결)는 D-1 사전과제에서 이미 끝났다는 전제로, 오늘은 **점검만** 합니다. 만약 설치가 안 돼 있다면 SA에게 손을 들어 알려주세요 — 헬프데스크가 별도로 돕습니다.
...
> 💡 **팁:** 막히면 혼자 3분 이상 헤매지 말고 옆 페어나 SA에게 손을 드세요. 환경 점검은 빨리 통과하고 본 실습으로 넘어가는 게 목표입니다.
...
도구 버전을 한 번 찍어 둡니다. SA가 전원이 같은 버전인지 확인하고, 문제 발생 시 헬프데스크가 참고합니다.
...
전원이 같은 버전을 쓰면 화면·동작이 일치해 SA가 진행을 맞추기 쉽습니다. 숫자가 정확히 `2.1.195`일 필요는 없고 **`2.x`대면 충분**합니다. 버전이 크게 다르면(예: 1.x) D-1 설치가 옛 버전이라는 뜻이니 헬프데스크가 업데이트를 돕습니다.
...
```text
안녕하세요! 오늘 잘 부탁드립니다.
이 폴더는 OB(발급사·1CB) 차지백 — 하나카드가 자사 카드홀더를 대리해
처음 청구(1CB)를 개시하는 업무를 돕기 위한 작업 폴더로,
자료 분석·사유코드 매칭·1CB 카드홀더레터 작성을 지원합니다.
```
...
| **리전 무시** | 응답은 오는데 리전이 서울이 아님 | 컴플라이언스상 **`ap-northeast-2`** 확인 — 다르면 SA에게 알리기 |
