# Lab 2 · 차지백 이해 스킬 직접 만들기

[← 이전: Lab 1 핵심 개념](01-concepts.md) · [🏠 목차](README.md) · [다음: Lab 3 서비스 정의 & 배포 준비 →](03-brainstorm-and-deploy.md)

이번 단계에서는 **"차지백(특히 인바운드 재반박)을 잘 이해하는 AI"를 우리 손으로 만듭니다.** 방법은 코딩이 아니라 **스킬(Skill)을 만드는 것**입니다. 스킬은 "이 도메인은 이렇게 일한다"를 적어 둔 **재사용 가능한 도메인 지식 묶음**입니다. 한 번 만들어 두면, 그 스킬을 불러오는 **어떤 AI 에이전트든** 차지백을 우리 규칙대로 이해하게 됩니다. 오늘 만드는 이 스킬은 단순 연습이 아니라, **Lab 4에서 AgentCore에 배포할 에이전트의 "두뇌"** 가 됩니다. 그래서 여기서 도메인 지식을 정확히 담는 것이 오늘 워크숍의 토대입니다.
...
- [ ] Lab 1(핵심 개념) 완료 — 인바운드/아웃바운드, 재반박/1CB, HITL의 큰 그림을 안다
- [ ] 터미널에서 `workshop/mvp/` 폴더로 이동 후 `claude` 실행한 상태
- [ ] `skill-creator` 스킬이 설치돼 있음 (Lab 0.5에서 설치 — 없으면 강사에게 알리기)
...
graph LR
    ...
    K -.->|Lab 4| AC["☁️ AgentCore에<br/>배포된 에이전트<br/>= 이 스킬이 두뇌"]
    style K fill:#e8f5e9
...
**오늘의 핵심 단계**입니다. 아래 차지백 도메인 지식을 한국어로 불러 주고, 스킬 본문에 반영시킵니다. 이 내용이 정확해야 Lab 4의 에이전트가 제대로 일합니다.
...
## ⚠️ 이 스킬이 Lab 4의 에이전트 "두뇌"가 됩니다

이 단계에서 만든 `chargeback-domain` 스킬은 단순 연습물이 아닙니다.

```mermaid
graph LR
    L2["Lab 2 (지금)<br/>차지백 스킬 제작<br/>chargeback-domain"]
    L3["Lab 3<br/>서비스 정의<br/>& 배포 준비"]
    L4["Lab 4<br/>AgentCore 배포<br/>에이전트 = 이 스킬이 두뇌"]
    L2 --> L3 --> L4
    L2 -.->|① brainstorming에 직접 주입<br/>= 설계 기준| L3
    L2 -.->|② 두뇌(페르소나)로 탑재| L4
...
이 스킬은 ① **Lab 3 brainstorming에 직접 주입**되어 우리가 만들 서비스(에이전트) 설계의 **기준**이 되고, ② **Lab 4에서 AgentCore 에이전트의 두뇌(페르소나)** 가 됩니다. 즉 Lab 2 → Lab 3 → Lab 4가 이 하나의 스킬로 **유기적으로 연결**됩니다.

> ✅ **핵심 연결:** 이 스킬은 ① Lab 3 brainstorming에 **직접 주입**되어 서비스 설계의 기준이 되고 ② Lab 4에서 **AgentCore 에이전트의 두뇌(페르소나)** 가 됩니다.

Lab 4에서 우리는 차지백 에이전트를 **Amazon Bedrock AgentCore**에 배포합니다. 그 에이전트가 "차지백을 어떻게 이해하고 처리하는가"는 **바로 이 스킬에 담긴 규칙**에서 나옵니다. 즉 **지금 스킬에 정확한 도메인 지식을 담을수록, Lab 4에서 배포될 에이전트가 똑똑해집니다.** 그래서 ②의 8가지 규칙(특히 방향 감지·매입사 응답요건·HITL·PII 익명화 가드레일)을 빠짐없이 담는 것이 오늘의 핵심입니다.
...
| `skill-creator`가 안 보임 | 스킬 미설치 | Lab 0.5의 설치를 확인 — 없으면 강사에게 손 들기 |
...
- **이 스킬은 ① Lab 3 brainstorming에 직접 주입돼 서비스 설계의 기준이 되고 ② Lab 4에서 AgentCore 에이전트의 두뇌(페르소나)** 가 된다.
...
> - **Lab 2→3→4 연결을 명시적으로** 짚으세요: 이 스킬은 ① Lab 3 brainstorming에 직접 주입돼 서비스 설계의 기준이 되고 ② Lab 4에서 AgentCore 에이전트의 두뇌(페르소나)가 됩니다.
...
> - ⚠️ Lab 4 연결을 **명시적으로** 말하세요: "오늘 만든 이 스킬이 오후 Lab 4에서 클라우드에 배포될 에이전트의 두뇌가 됩니다."
>
> **시간 관리 (강사 기준)**
...
> - **Q. 이 스킬을 나중에 또 쓰나요?** A. 네. Lab 4에서 AgentCore에 배포할 에이전트의 두뇌가 바로 이 스킬입니다. 그래서 지금 정확히 만드는 게 중요합니다.
> - **Q. 실제 고객 데이터를 예시로 써도 되나요?** A. 절대 안 됩니다. POC라 익명 더미(예: `1111-1111-1111-1111`)만 씁니다 — 그 규칙도 스킬에 명시돼 있습니다.

## 다음 단계

이제 차지백을 이해하는 **두뇌(스킬)** 를 우리 손으로 만들었습니다. Lab 3에서는 이 두뇌를 쓸 **서비스(에이전트)를 어떻게 정의할지** 브레인스토밍하고, 클라우드(AgentCore)에 **배포할 준비**를 합니다.
